<template>
  ...
  <aside
    ...
    :class="[
      collapsed ? 'w-16' : 'w-64',
      'bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-xl border-r border-gray-200 dark:border-gray-800'
    ]"
  >
    <!-- Logo Section -->
          ...
          >
            <router-link
              ...
              :class="[
                isActiveRoute(item.name)
                  ? 'bg-harvest-500/20 text-harvest-600 dark:text-harvest-400'
            ...
            >
              <UIcon
                ...
                :class="[isActiveRoute(item.name) ? 'text-harvest-600 dark:text-harvest-400' : 'text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white']"
              />
              <Transition name="fade">
    ...
    <div class="p-2 border-t border-gray-200 dark:border-gray-800">
      <button
        class="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white transition-colors"
        @click="emit('toggle')"
      >
        <UIcon
  ...
  <Transition name="slide">
    <aside
      v-if="mobileOpen"
      class="fixed left-0 top-0 h-full w-64 z-50 lg:hidden bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-xl border-r border-gray-200 dark:border-gray-800"
    >
      <!-- Close Button -->
      <div class="h-16 flex items-center justify-between px-4 border-b border-gray-200 dark:border-gray-800">
        <div class="flex items-center gap-3">
          <img
            ...
            class="w-8 h-8 object-contain"
          />
          <span class="text-lg font-bold text-gray-900 dark:text-white">日沐 後台</span>
        </div>
        <UButton
